...
pub fn run() {
    let octopi = Octopi::new(parse_energies(r"
        7313511551
        3724855867
        2374331571
        4438213437
        ...
        8812617112
    "));
    println!("Day 11: After 100 days there have been {} flashes", octopi.take(100).sum::<usize>());
}
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        loop {
            ...
            self.energies.iter().enumerate()
                .flat_map(|(row, entries)| entries.iter().enumerate()
                          .map(move |(col, e)| (row, col, e)))
                .filter(|(_row, _col, e)| **e >= 10)
                .for_each(|(row, col, _e)| {
                    if who_flashed.contains(&(row, col)) { return; }
                    ...
                    neighbors.push((row+1, col+1));
                    neighbors.iter()
                        .filter(|(r, c)| *r >= 0 && *r < self.energies.len() as i32 &&
                                         *c >= 0 && *c < self.energies[*r as usize].len() as i32)
                        .for_each(|(r, c)| bump_neighbors.push((*r as usize, *c as usize)));
                });
            bump_neighbors.iter()
                .for_each(|(row, col)| self.energies[*row][*col] += 1);
            if num_flashed_in_step <= 0 { break; }
        }
...
mod test {
    ...
    fn test_count_octopi_flashes_small() {
        let mut octopi = Octopi::new(parse_energies(r"
        11111
        19991
        19191
        19991
